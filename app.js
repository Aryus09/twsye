...
// === 👤 MENU AKUN SAYA (DENGAN TOMBOL HAPUS EXP) ===
bot.action('akun_saya', async (ctx) => {
  try {
    await ctx.answerCbQuery().catch(() => {}); // Hilangkan icon loading
    const userId = ctx.from.id;

    // Ambil data akun dari database, gabungkan dengan nama server
    const query = `
    ...
    db.all(query, [userId], async (err, accounts) => {
      if (err) {
        logger.error('Gagal mengambil data akun VPN:', err.message);
        return ctx.reply('❌ Terjadi kesalahan saat mengambil data akun kamu.');
      }
      ...
      // Jika user punya akun, kita susun pesannya
      let pesan = `📝 <b>DAFTAR AKUN ZIVPN KAMU</b>\n──────────────────\n\n`;
      ...
      accounts.forEach((acc, index) => {
        ...
        // Deteksi jika ada akun yang expired
        if (isExpired) adaExpired = true;
        
        const statusIcon = isExpired ? '🔴 Expired' : '🟢 Aktif';

        pesan += `<b>${index + 1}. Server:</b> ${acc.nama_server || 'Unknown'}\n`;
        pesan += `🌐 <b>Host:</b> <code>${acc.domain}</code>\n`;
        pesan += `🔑 <b>Password:</b> <code>${acc.password}</code>\n`;
      ...
      // Munculkan tombol Hapus hanya jika ada akun yang expired
      if (adaExpired) {
        keyboard.push([{ text: '🧹 Hapus Akun Expired', callback_data: 'hapus_akun_expired' }]);
      }
      
      // Tombol kembali selalu ada di bawah
      keyboard.push([{ text: '🔙 Kembali ke Menu Utama', callback_data: 'send_main_menu' }]);
