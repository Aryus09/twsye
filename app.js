...
// ... (Bagian atas file app.js kamu tidak berubah, biarkan saja) ...
...
bot.on('text', async (ctx) => {
  ...
  if (state.step?.startsWith('username_del_')) {
    ...
    if (!/^[a-zA-Z0-9]{3,20}$/.test(username)) {
      return ctx.reply('❌ *Username tidak valid. Gunakan huruf dan angka (3–20 karakter).*', { parse_mode: 'Markdown' });
    }
    ...
    db.get(`
      SELECT v.expired_at, s.harga 
      FROM vpn_accounts v 
      JOIN Server s ON v.server_id = s.id 
      WHERE v.user_id = ? AND v.password = ? AND v.server_id = ?
    `, [ctx.from.id, username, serverId], async (errRow, row) => {
      let refundAmount = 0;
      let sisaHari = 0;
      ...
      try {
        ...
        await ctx.reply(msg, { parse_mode: 'Markdown' });
        logger.info(`✅ Akun ${type} berhasil dihapus oleh user ${ctx.from.id}`);
        ...
        if (!msg.includes('❌')) {
          ...
          if (refundAmount > 0) {
            db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [refundAmount, ctx.from.id], (updateErr) => {
              if (!updateErr) {
                ctx.reply(`💸 *Refund Saldo Berhasil!*\n\n👤 Akun: \`${username}\`\n⏳ Sisa Waktu: *${sisaHari} Hari*\n💰 Saldo Kembali: *Rp ${refundAmount.toLocaleString('id-ID')}*`, { parse_mode: 'Markdown' });
              }
            });
          }
        }
  ...
  // === BIKIN/PERPANJANG AKUN: PASSWORD ===
  if (state.step?.startsWith('password_')) {
    state.password = text;

    if (!state.password || state.password.length < 3 || /[^a-zA-Z0-9]/.test(state.password)) {
      return ctx.reply('❌ *Password tidak valid (Maks 3 Karakter, Huruf & Angka saja).*', { parse_mode: 'Markdown' });
    }
  ...
  // === BIKIN/PERPANJANG AKUN: MASA AKTIF & PROSES ===
  if (state.step?.startsWith('exp_')) {
    ...
    if (isNaN(exp) || exp <= 0 || exp > 365) {
      return ctx.reply('❌ *Masa aktif tidak valid (Maks 365 hari).*', { parse_mode: 'Markdown' });
    }
    state.exp = exp;
    ...
    db.get('SELECT quota, iplimit, harga FROM Server WHERE id = ?', [state.serverId], async (err, server) => {
      ...
      const totalHarga = server.harga * state.exp; 
      ...
      db.get('SELECT saldo FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
        ...
        if (user.saldo < totalHarga) return ctx.reply('❌ *Saldo Anda tidak mencukupi.*', { parse_mode: 'Markdown' });
        ...
        if (action === 'create') {
          ...
          logger.info(`Account created for ${ctx.from.id}`);
          const maskedpassword = password.length > 1 ? `${password.slice(0, 1)}${'x'.repeat(password.length - 1)}` : password;
          await bot.telegram.sendMessage(GROUP_ID, `<blockquote>📢 <b>Account Created</b>\n━━━━━━━━━━━━━━━━━━━━\n👤 <b>User:</b> ${ctx.from.first_name} (${ctx.from.id})\n🧾 <b>Type:</b> ${type.toUpperCase()}\n📛 <b>Password:</b> ${maskedpassword}\n📆 <b>Expired:</b> ${exp || '0'}\n💾 <b>Quota:</b> ${quota || '0'}\n🌐 <b>Server ID:</b> ${serverId}\n━━━━━━━━━━━━━━━━━━━━</blockquote>`, { parse_mode: 'HTML' });
        } else if (action === 'renew') {
          if (type === 'ssh') {
            msg = await renewssh(username, password, exp, iplimit, serverId);
            await recordAccountTransaction(ctx.from.id, 'ssh');
          }
          logger.info(`Account renewed for ${ctx.from.id}`);
          const maskedpassword = password.length > 1 ? `${password.slice(0, 1)}${'x'.repeat(password.length - 1)}` : password;
          await bot.telegram.sendMessage(GROUP_ID, `<blockquote>♻️ <b>Account Renewed</b>\n━━━━━━━━━━━━━━━━━━━━\n👤 <b>User:</b> ${ctx.from.first_name} (${ctx.from.id})\n🧾 <b>Type:</b> ${type.toUpperCase()}\n📛 <b>Password:</b> ${maskedpassword}\n📆 <b>New Expiry:</b> ${exp || '0'}\n💾 <b>Quota:</b> ${quota || '0'}\n🌐 <b>Server ID:</b> ${serverId}\n━━━━━━━━━━━━━━━━━━━━</blockquote>`, { parse_mode: 'HTML' });
        }
        ...
        db.get('SELECT domain, nama_server FROM Server WHERE id = ?', [serverId], async (err, rowServer) => {
          if (!err && rowServer) {
            ...
            if (action === 'create') {
              db.run('INSERT INTO vpn_accounts (user_id, server_id, domain, password, expired_at) VALUES (?, ?, ?, ?, ?)', [ctx.from.id, serverId, rowServer.domain, state.password, expTimestamp]);
            } else if (action === 'renew') {
              db.get('SELECT id, expired_at FROM vpn_accounts WHERE user_id = ? AND password = ? AND server_id = ?', [ctx.from.id, state.password, serverId], (err, acc) => {
                if (acc) {
  ...
  // === MENU TAMBAH SERVER (ADMIN) ===
  if (state.step === 'addserver') {
    const domain = text;
    if (!domain) {
      await ctx.reply('⚠️ *Domain tidak boleh kosong.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_auth') {
    const auth = text;
    if (!auth) {
      await ctx.reply('⚠️ *Auth tidak boleh kosong.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_nama_server') {
    ...
    if (!nama_server) {
      await ctx.reply('⚠️ *Nama server tidak boleh kosong.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_quota') {
    ...
    if (isNaN(quota)) {
      await ctx.reply('⚠️ *Quota tidak valid.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_iplimit') {
    ...
    if (isNaN(iplimit)) {
      await ctx.reply('⚠️ *Limit IP tidak valid.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_batas_create_akun') {
    ...
    if (isNaN(batas_create_akun)) {
      await ctx.reply('⚠️ *Batas create akun tidak valid.*', { parse_mode: 'Markdown' });
      return;
    }
  ...
  if (state.step === 'addserver_harga') {
    ...
    if (isNaN(harga) || harga <= 0) {
      await ctx.reply('⚠️ *Harga tidak valid.*', { parse_mode: 'Markdown' });
      return;
    }
...
// === ACTION GENERATE FILE .ZIV ===
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  const password = ctx.match[1];
  const userId = ctx.from.id;

  // Cari data domain di database berdasarkan password & user_id
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [password, userId], async (err, acc) => {
    if (err || !acc) {
      return ctx.answerCbQuery('❌ Gagal membuat file. Akun tidak ditemukan.', { show_alert: true });
    }

    // Munculkan notif loading di atas layar
    await ctx.answerCbQuery('Sedang membuat config .ziv...');

    try {
      const crypto = require('crypto');
      
      // 1. JSON diubah biar rapi ke bawah (Pretty Print) pakai parameter (null, 2)
      const rawConfig = JSON.stringify({
         "Author": "https://t.me/xd_system101",
         "ok": true,
         "ext": "ziv",
         "data": {
             "sshPortaLocal": "1080",
             ...
             "tcpnodelay": "0",
             "injectionmode": "1",
             "currentpsiprotocol": "0",
             "udpForward": "0",
             "usarDefaultPayload": "0",
             "tunnelType": "4",
             "udpwindow": "2",
             "udpserver": acc.domain,
             "gamma": "0",
             "file.askUser": "0",
             "psiauth": "",
             "litehost": "",
      ...
      }, null, 2); // <=== INI OBATNYA (null, 2) BIAR SUSUNANNYA RAPI

      const secretPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12); // <=== UBAH JADI 12 BYTE (Standar Android GCM)
      const key = crypto.pbkdf2Sync(secretPass, salt, 1000, 16, 'sha256');
      
      const cipher = crypto.createCipheriv('aes-128-gcm', key, nonce);
      let ciphertext = cipher.update(rawConfig, 'utf8');
      ciphertext = Buffer.concat([ciphertext, cipher.final()]);
      const tag = cipher.getAuthTag();
      
      const blob = Buffer.concat([ciphertext, tag]);
      
      // Tambahkan \n di paling belakang (beberapa app VPN wajib ada Enter di akhir file)
      const finalZivString = `${salt.toString('base64')}.${nonce.toString('base64')}.${blob.toString('base64')}\n`;

      const fileBuffer = Buffer.from(finalZivString, 'utf-8');
      const fileName = `Aryus_${password}.ziv`;

      await ctx.replyWithDocument(
        { source: fileBuffer, filename: fileName },
        {
          caption: `📄 *Config ZIVPN Auto-Generated*\nUntuk Host: \`${acc.domain}\`\n\nSilakan import file ini langsung ke aplikasi ZIVPN`,
          parse_mode: 'Markdown'
        }
      );
    } catch (e) {
      console.error("Error generating ZIV:", e);
      ctx.reply("❌ Gagal membuat file .ziv, sistem error.");
    }
