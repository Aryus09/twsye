...
//
// === ACTION GENERATE FILE .ZIV V3 (FINAL REPAIR) ===
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [pZiv, uZiv], async (err, row) => {
    if (err || !row) return ctx.answerCbQuery('❌ Akun tidak ditemukan.', { show_alert: true });

    await ctx.answerCbQuery('🚀 Menjahit Config .ZIV...');

    try {
      const crypto = require('crypto');
      
      // KITA PAKAI NAMA VARIABEL SUPER UNIK BIAR GAK BISA DICOPIER/DIBENTROKKAN
      const r_Config_Aryus = {
        "Author": "https://t.me/aryusZIVPN",
        "ok": true,
        "ext": "ziv",
        "data": {
          "sshPortaLocal": "1080",
      ...
      // PAKSA JADI STRING DAN CEK PANJANGNYA
      const finalJsonString = JSON.stringify(r_Config_Aryus, null, 2);

      const sPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const saltZ = crypto.randomBytes(16);
      const nonceZ = crypto.randomBytes(12); // WAJIB 12 BYTE
      const keyZ = crypto.pbkdf2Sync(sPass, saltZ, 1000, 16, 'sha256');
      
      const cipherZ = crypto.createCipheriv('aes-128-gcm', keyZ, nonceZ);
      let cipherTextZ = cipherZ.update(finalJsonString, 'utf8');
      cipherTextZ = Buffer.concat([cipherTextZ, cipherZ.final()]);
      const tagZ = cipherZ.getAuthTag();
      
      const blobZ = Buffer.concat([cipherTextZ, tagZ]);
      const resultZiv = `${saltZ.toString('base64')}.${nonceZ.toString('base64')}.${blobZ.toString('base64')}\n`;

      await ctx.replyWithDocument(
        { source: Buffer.from(resultZiv, 'utf-8'), filename: `Aryus_${pZiv}.ziv` },
        { caption: `✅ *Config ZIVPN Berhasil!*\nHost: \`${row.domain}\`\n\nIsi file sudah diperbaiki.`, parse_mode: 'Markdown' }
      );
    } catch (e) {
      console.error("Gagal buat ZIV:", e);
      ctx.reply("❌ Sistem error saat merakit file.");
    }
  });
});
