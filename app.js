...
//
  if (state.step?.startsWith('username_del_')) {
    ...
    if (!/^[a-zA-Z0-9]{3,20}$/.test(username)) {
      return ctx.reply('❌ *Username tidak valid. (3–20 karakter).*', { parse_mode: 'Markdown' });
    }

    const { type, serverId } = state;
    delete userState[ctx.chat.id];

    // 1. AMBIL DATA DULU DARI DATABASE (PENTING SEBELUM HAPUS DI SERVER)
    db.get(`
    ...
    `, [ctx.from.id, username, serverId], async (errRow, row) => {
      ...
      // 2. HITUNG SALDO REFUND BERDASARKAN SISA HARI
      ...
      try {
        ...
        // 3. JALANKAN PERINTAH HAPUS KE VPS
        let msg = await delFunctions[type](username, password, exp, iplimit, serverId);

        // 4. CEK JIKA PENGHAPUSAN SUKSES (TIDAK ADA EMOJI ❌)
        if (!msg.includes('❌')) {
          // Hapus data akun dari database bot
          db.run('DELETE FROM vpn_accounts WHERE user_id = ? AND password = ? AND server_id = ?', [ctx.from.id, username, serverId]);
          
          // Kirim laporan sukses hapus dari server
          await ctx.reply(msg, { parse_mode: 'Markdown' });

          // 5. PROSES REFUND SALDO KE USER
          if (refundAmount > 0) {
            db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [refundAmount, ctx.from.id], (updateErr) => {
              if (!updateErr) {
                ctx.reply(`💸 *Refund Saldo Berhasil!*\n\n` +
                          ...
                          { parse_mode: 'Markdown' });
                logger.info(`Refund sukses Rp${refundAmount} ke user ${ctx.from.id}`);
              } else {
          ...
          // Jika server memberikan respon gagal (ada logo ❌)
          await ctx.reply(msg, { parse_mode: 'Markdown' });
        }
