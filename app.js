...
db.run('UPDATE users SET saldo = saldo - ? WHERE user_id = ?', [totalHarga, ctx.from.id], (err) => {
  if (err) {
   ...
   // === SIMPAN DATA AKUN KE DATABASE DAN KIRIM PESAN DENGAN TOMBOL SALIN ===
   db.get('SELECT domain, nama_server FROM Server WHERE id = ?', [serverId], async (err, rowServer) => {
     if (!err && rowServer) {
       ...
       // Format teks yang akan disalin ketika tombol ditekan
       const copyTextData = `Host: ${rowServer.domain}\nPassword: ${state.password}`;

       // Kirim pesan beserta tombol Salin
       await ctx.reply(msg, { 
         parse_mode: 'Markdown',
         reply_markup: {
           inline_keyboard: [
             [
               { 
                 text: '📋 Salin Host & Password', 
                 copy_text: { text: copyTextData } 
               }
             ]
           ]
         }
       ...
       // Fallback jika terjadi error saat mengambil data server
       await ctx.reply(msg, { parse_mode: 'Markdown' });
     }
     
     // Hapus state pengguna setelah proses selesai
     delete userState[ctx.chat.id];
   });
   // ====================================
//SALDO DATABES
