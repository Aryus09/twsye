...
bot.on('text', async (ctx) => {
  ...
  if (state.step?.startsWith('username_del_')) {
    ...
    db.get(`SELECT v.expired_at, s.harga FROM vpn_accounts v JOIN Server s ON v.server_id = s.id WHERE v.user_id = ? AND v.password = ? AND v.server_id = ?`, 
    [ctx.from.id, username, serverId], async (errRow, row) => {
      let refundAmount = 0;
      if (!errRow && row && row.expired_at > Date.now()) {
        const sisaHari = Math.floor((row.expired_at - Date.now()) / 86400000);
        if (sisaHari > 0) refundAmount = sisaHari * row.harga;
      }

      let msg = await delssh(username, 'none', 'none', 'none', serverId);
      await ctx.reply(msg, { parse_mode: 'Markdown' });
      ...
      if (!msg.includes('❌')) {
        ...
        if (refundAmount > 0) {
          ...
          ctx.reply(`💸 *Refund Berhasil!* Rp ${refundAmount.toLocaleString()}`, { parse_mode: 'Markdown' });
        }
      }
    });
    return;
  ...
  if (state.step?.startsWith('password_')) {
    if (!text || text.length < 3 || /[^a-zA-Z0-9]/.test(text)) {
      return ctx.reply('❌ *Password tidak valid (Min 3 Karakter, Huruf & Angka).*');
    }
    state.password = text;
    state.step = `exp_${state.action}_${state.type}`;
  ...
  if (state.step?.startsWith('exp_')) {
    ...
    if (isNaN(exp) || exp <= 0 || exp > 365) return ctx.reply('❌ *Masa aktif 1-365 hari.*');
    state.exp = exp;

    db.get('SELECT domain, harga, quota, iplimit FROM Server WHERE id = ?', [state.serverId], async (err, server) => {
      ...
      db.get('SELECT saldo FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
        if (!user || user.saldo < totalHarga) return ctx.reply('❌ *Saldo tidak cukup.*');

        let msg = (state.action === 'create') 
          ? await createssh(state.username, state.password, state.exp, server.iplimit, state.serverId)
          : await renewssh(state.username, state.password, state.exp, server.iplimit, state.serverId);

        if (msg.includes('❌')) return ctx.reply(msg, { parse_mode: 'Markdown' });

        db.run('UPDATE users SET saldo = saldo - ? WHERE user_id = ?', [totalHarga, ctx.from.id]);
        const expTS = Date.now() + (state.exp * 86400000);
        
        db.run('INSERT INTO vpn_accounts (user_id, server_id, domain, password, expired_at) VALUES (?, ?, ?, ?, ?)', 
        [ctx.from.id, state.serverId, server.domain, state.password, expTS], async (err) => { 
          
          await ctx.reply(msg, { 
            parse_mode: 'Markdown',
            reply_markup: {
              inline_keyboard: [
                [{ text: '📋 Salin Host', copy_text: { text: server.domain } }, { text: '🔑 Salin Pass', copy_text: { text: state.password } }],
