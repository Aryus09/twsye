...
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [password, userId], async (err, acc) => {
    ...
    try {
      ...
      // === 1. RAKIT SEMUA DATA KE SATU VARIABEL (JANGAN DIPISAH) ===
      const jsonObjek = {
        ...
        "data": {
          ...
          "gamma": "0",
          "file.askUser": "0",
          "file.msg": "BOT ARYUS ZIVPN FIXED",
          "dnsResolver": "8.8.8.8",
          "psitimeout": "60",
          "sshAddress": ":@:"
        }
      ...
      // === 2. UBAH JADI STRING (null, 2 WAJIB UNTUK ZIVPN) ===
      const rawConfig = JSON.stringify(jsonObjek, null, 2);

      const secretPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12); // Standar Android
