...
// Update file heartbeat setiap 10 detik
setInterval(() => {
    try {
        fs.writeFileSync('/root/bot_heartbeat.txt', Date.now().toString());
    } catch (err) {
        console.error("Gagal menulis heartbeat", err);
    }
}, 10000);
...
const db = new sqlite3.Database('./sellzivpn.db', (err) => {
  if (err) {
    ...
    // Tambahkan 2 baris ini untuk mengatasi SQLITE_BUSY
    db.run("PRAGMA journal_mode = WAL;"); 
    db.run("PRAGMA busy_timeout = 5000;"); // Set antrean nunggu 5 detik kalau db lagi dipakai
  }
});
...
db.run(
  ...
  (err) => {
   ...
   db.run(`CREATE TABLE IF NOT EXISTS vpn_accounts (
     id INTEGER PRIMARY KEY AUTOINCREMENT,
     user_id INTEGER,
     server_id INTEGER,
     domain TEXT,
     ...
     if (err) {
       logger.error('Kesalahan membuat tabel vpn_accounts:', err.message);
     } else {
       logger.info('vpn_accounts table created or already exists');
     }
   });
...
let keyboard;
  keyboard = [
    ...
    [
     { text: '📋 Akun Saya', callback_data: 'akun_saya' }
    ],
    [
...
//
//
  // === BAGIAN PERBAIKAN DELETE & REFUND (ANTI ERROR) ===
  if (state.step?.startsWith('username_del_')) {
    ...
    db.get(
      'SELECT v.expired_at, s.harga FROM vpn_accounts v JOIN Server s ON v.server_id = s.id WHERE v.user_id = ? AND v.password = ? AND v.server_id = ?',
      [ctx.from.id, username, serverId],
      async (errRow, row) => {
        
        let refundAmount = 0;
        let sisaHari = 0;

        if (!errRow && row && row.expired_at) {
          const now = Date.now();
          if (row.expired_at > now) {
            sisaHari = Math.floor((row.expired_at - now) / (1000 * 60 * 60 * 24));
            if (sisaHari > 0) {
              refundAmount = sisaHari * row.harga;
            }
          }
        }
        ...
        try {
          ...
          // 2. Hapus akun di server
          let msg = await delFunctions[type](username, password, exp, iplimit, serverId);

          if (!msg.includes('❌')) {
            // Hapus dari database lokal
            db.run('DELETE FROM vpn_accounts WHERE user_id = ? AND password = ? AND server_id = ?', [ctx.from.id, username, serverId]);
            
            await ctx.reply(msg, { parse_mode: 'Markdown' });

            // 3. Proses Refund
            if (refundAmount > 0) {
              db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [refundAmount, ctx.from.id], (updateErr) => {
                if (!updateErr) {
                  // Format teks dibikin aman pakai kutip ganda dan plus (+)
                  const pesanRefund = "💸 *Refund Saldo Berhasil!*\n\n👤 Akun: `" + username + "`\n⏳ Sisa: *" + sisaHari + " Hari*\n💰 Saldo Kembali: *Rp " + refundAmount.toLocaleString('id-ID') + "*";
                  
                  ctx.reply(pesanRefund, { parse_mode: 'Markdown' });
                  logger.info("Refund sukses Rp" + refundAmount + " ke " + ctx.from.id);
                }
              });
            }
          } else {
          ...
          await ctx.reply("❌ *Terjadi kesalahan sistem.*", { parse_mode: 'Markdown' });
        }
      }
    ); // Tutup db.get
    return;
...
db.run('UPDATE users SET saldo = saldo - ? WHERE user_id = ?', [totalHarga, ctx.from.id], (err) => {
  if (err) {
   ...
   db.get('SELECT domain, nama_server FROM Server WHERE id = ?', [serverId], async (err, rowServer) => {
     if (!err && rowServer) {
       // Konversi masa aktif (hari) ke milidetik (timestamp)
       const expTimestamp = Date.now() + (state.exp * 24 * 60 * 60 * 1000); 
       
       if (action === 'create') {
         db.run('INSERT INTO vpn_accounts (user_id, server_id, domain, password, expired_at) VALUES (?, ?, ?, ?, ?)',
           [ctx.from.id, serverId, rowServer.domain, state.password, expTimestamp]
         );
       } else if (action === 'renew') {
         db.get('SELECT id, expired_at FROM vpn_accounts WHERE user_id = ? AND password = ? AND server_id = ?', 
           [ctx.from.id, state.password, serverId], (err, acc) => {
           if (acc) {
             let baseTime = acc.expired_at > Date.now() ? acc.expired_at : Date.now();
             let newExp = baseTime + (state.exp * 24 * 60 * 60 * 1000);
             db.run('UPDATE vpn_accounts SET expired_at = ? WHERE id = ?', [newExp, acc.id]);
           } else {
             db.run('INSERT INTO vpn_accounts (user_id, server_id, domain, password, expired_at) VALUES (?, ?, ?, ?, ?)',
               [ctx.from.id, serverId, rowServer.domain, state.password, expTimestamp]
             );
       ...
       // Kirim pesan sukses BESERTA tombol rakit file .ZIV
       await ctx.reply(msg, { 
         parse_mode: 'Markdown',
         reply_markup: {
           inline_keyboard: [
             [
               { text: '📋 Salin Host', copy_text: { text: rowServer.domain } },
               { text: '🔑 Salin Password', copy_text: { text: state.password } }
             ],
             [
               { text: '📁 Buat File .ZIV', callback_data: `get_ziv_${state.password}` }
             ]
           ]
         }
       });
       ...
       // Fallback jika database gagal, tetap kirim pesan sukses tapi tanpa tombol
       await ctx.reply(msg, { parse_mode: 'Markdown' });
     }
...
//
// === ACTION GENERATE FILE .ZIV SAKTI (ANTI-KOSONG) ===
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  const pZiv = ctx.match[1];
  const uZiv = ctx.from.id;

  // 1. Cek apakah user adalah reseller
  const isRessel = await isUserReseller(uZiv);
  
  // 2. Ambil nama user (bisa dari username atau first_name) dan bersihkan karakter unik agar aman jadi nama file
  let rawName = ctx.from.username || ctx.from.first_name || "Reseller";
  let cleanName = rawName.replace(/[^a-zA-Z0-9]/g, ''); 
  if (!cleanName) cleanName = "Reseller";

  // 3. Tentukan Nama File dan Pesan (file.msg)
  // Jika reseller, pakai namanya. Jika bukan, pakai default AryusZIVPN
  let fileNamePrefix = isRessel ? cleanName : "AryusZIVPN";
  let fileMsg = isRessel ? cleanName : "ARYUS ZIVPN #t.me/AryusSSH_Bot";

  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [pZiv, uZiv], async (err, row) => {
    if (err || !row) return ctx.answerCbQuery('❌ Akun tidak ditemukan.');

    try {
      const crypto = require('crypto');

      // === RAKIT FORMAT XML ORIGINAL ===
      // Perhatikan bagian <entry key="file.msg"> sudah diubah jadi dinamis
      const xmlData = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE properties SYSTEM "http://java.sun.com/dtd/properties.dtd">
<properties>
<comment>Configuration File</comment>
<entry key="vpnshare">0</entry>
<entry key="litehost"></entry>
<entry key="v2rayprofile"></entry>
<entry key="injectionmode">1</entry>
<entry key="psiproxy"></entry>
<entry key="nameServer"></entry>
<entry key="publickey"></entry>
<entry key="customdns">0</entry>
<entry key="proxyRemote"></entry>
<entry key="psiauth"></entry>
<entry key="boostslowdns">0</entry>
<entry key="tcpnodelay">0</entry>
<entry key="customdnsvalue2"></entry>
<entry key="tunnelType">4</entry>
<entry key="customdnsvalue1"></entry>
<entry key="dnsForward">0</entry>
<entry key="file.appVersionCode">24</entry>
<entry key="udpForward">1</entry>
<entry key="sniHost"></entry>
<entry key="file.askLogin">0</entry>
<entry key="psitimeout">60</entry>
<entry key="file.askHwidLocal">0</entry>
<entry key="udpcustomserver">1</entry>
<entry key="gamma">1</entry>
<entry key="sshServer"></entry>
<entry key="psiserverentry"></entry>
<entry key="udpauth">${pZiv}</entry>
<entry key="file.askUser">0</entry>
<entry key="udpserver">${row.domain}</entry>
<entry key="file.askHwid">0</entry>
<entry key="beta">2000000000000</entry>
<entry key="dnsResolver">8.8.8.8</entry>
<entry key="proxyPayload"></entry>
<entry key="sshPortaLocal">1080</entry>
<entry key="udpwindow">2</entry>
<entry key="udpResolver">127.0.0.1:7300</entry>
<entry key="file.hwids"></entry>
<entry key="sshAllinOne"></entry>
<entry key="psibuiltin">0</entry>
<entry key="psiprotocol">0</entry>
<entry key="alpha">1</entry>
<entry key="usarDefaultPayload">0</entry>
<entry key="currentpsiprotocol">0</entry>
<entry key="speedup"></entry>
<entry key="speeddown"></entry>
<entry key="file.msg">${fileMsg}</entry>
</properties>`;

      const secretPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12);
      
      // LOGIKA PBKDF2 SESUAI PYTHON (HMAC-SHA256)
      const key = crypto.pbkdf2Sync(secretPass, salt, 1000, 16, 'sha256');
      
      const cipher = crypto.createCipheriv('aes-128-gcm', key, nonce);
      let ciphertext = cipher.update(xmlData, 'utf8');
      ciphertext = Buffer.concat([ciphertext, cipher.final()]);
      const tag = cipher.getAuthTag();
      
      const blob = Buffer.concat([ciphertext, tag]);
      const finalZiv = `${salt.toString('base64')}.${nonce.toString('base64')}.${blob.toString('base64')}\n`;

      // 4. Ubah filename pakai variabel dinamis
      await ctx.replyWithDocument(
        { source: Buffer.from(finalZiv, 'utf-8'), filename: `${fileNamePrefix}_${pZiv}.ziv` },
        { caption: `✅ *Config ZIVPN Original Berhasil!*\nHost: \`${row.domain}\``, parse_mode: 'Markdown' }
      );
    } catch (e) {
      console.error(e);
      ctx.reply("❌ Error Enkripsi");
    }
  });
});
...
// === FUNGSI UTAMA UNTUK MENAMPILKAN AKUN (DENGAN HALAMAN) ===
async function showAkunSaya(ctx, page = 0) {
  try {
    const userId = ctx.from.id;
    const limit = 10; // Jumlah akun per halaman
    
    const query = `
      SELECT v.password, v.domain, s.nama_server, v.expired_at 
      FROM vpn_accounts v 
      LEFT JOIN Server s ON v.server_id = s.id 
      WHERE v.user_id = ?
    ...
    if (!accounts || accounts.length === 0) {
      return ctx.editMessageText('📭 <b>Kamu belum memiliki akun ZIVPN yang aktif.</b>\n\nSilakan klik tombol "Buat Akun" di menu utama.', {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 Kembali ke Menu Utama', callback_data: 'send_main_menu' }]] }
      });
    }
    ...
    const currentAccounts = accounts.slice(currentPage * limit, (currentPage + 1) * limit);

    let pesan = `📝 <b>DAFTAR AKUN ZIVPN KAMU (Hal ${currentPage + 1}/${totalPages})</b>\n──────────────────\n\n`;

    currentAccounts.forEach((acc, index) => {
      const expDate = new Date(acc.expired_at).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' });
      const isExpired = acc.expired_at < Date.now();
      const statusIcon = isExpired ? '🔴 Expired' : '🟢 Aktif';

      // Escape karakter HTML
      const safeServer = (acc.nama_server || 'Unknown').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const safeDomain = (acc.domain || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const safePassword = (acc.password || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');

      const realIndex = (currentPage * limit) + index + 1;
      pesan += `<b>${realIndex}. Server:</b> ${safeServer}\n`;
      pesan += `🌐 <b>Host:</b> <code>${safeDomain}</code>\n`;
      pesan += `🔑 <b>Password:</b> <code>${safePassword}</code>\n`;
      pesan += `📅 <b>Expired:</b> ${expDate}\n`;
      pesan += `⚡ <b>Status:</b> ${statusIcon}\n`;
      pesan += `──────────────────\n`;
    });

    // Susun tombol (Inline Keyboard)
    let keyboard = [];

    // Tombol Navigasi Halaman
    const navButtons = [];
    if (totalPages > 1) {
      if (currentPage > 0) navButtons.push({ text: '⬅️ Back', callback_data: `akun_saya_page_${currentPage - 1}` });
      if (currentPage < totalPages - 1) navButtons.push({ text: '➡️ Next', callback_data: `akun_saya_page_${currentPage + 1}` });
    }
    if (navButtons.length > 0) keyboard.push(navButtons);

    // Cek apakah ada akun yang expired dari KESELURUHAN akun (bukan cuma di halaman ini)
    const adaExpired = accounts.some(acc => acc.expired_at < Date.now());
    if (adaExpired) {
      keyboard.push([{ text: '🧹 Hapus Semua Akun Expired', callback_data: 'hapus_akun_expired' }]);
    }
    ...
    logger.error('Error menu Akun Saya:', error);
    try {
      await ctx.reply('❌ Terjadi kesalahan sistem saat mengambil data akun kamu.');
    } catch (e) {}
  }
}

// === HANDLER TOMBOL AKUN SAYA UTAMA (HALAMAN 1) ===
bot.action('akun_saya', async (ctx) => {
  await ctx.answerCbQuery().catch(() => {});
  await showAkunSaya(ctx, 0); // Mulai dari halaman 0 (pertama)
});

// === HANDLER TOMBOL NEXT/PREV HALAMAN AKUN ===
bot.action(/akun_saya_page_(\d+)/, async (ctx) => {
  await ctx.answerCbQuery().catch(() => {});
  const page = parseInt(ctx.match[1]);
  await showAkunSaya(ctx, page); // Buka sesuai nomor halaman yang diklik
});


// === 🧹 HANDLER HAPUS AKUN EXPIRED ===
bot.action('hapus_akun_expired', async (ctx) => {
  try {
    const userId = ctx.from.id;
    const waktuSekarang = Date.now();

    // Hapus akun dari database SQLite yang expired_at-nya lebih kecil dari waktu saat ini
    db.run('DELETE FROM vpn_accounts WHERE user_id = ? AND expired_at < ?', [userId, waktuSekarang], async function(err) {
      if (err) {
        logger.error('Gagal hapus akun expired:', err.message);
        return ctx.answerCbQuery('❌ Gagal membersihkan akun expired.', { show_alert: true });
      }

      const jumlahDihapus = this.changes; // Mengambil jumlah baris yang berhasil dihapus

      if (jumlahDihapus > 0) {
        await ctx.answerCbQuery(`✅ Berhasil membersihkan ${jumlahDihapus} akun expired!`, { show_alert: true });
        
        // Refresh menu ke Main Menu
        await sendMainMenu(ctx);
      } else {
        await ctx.answerCbQuery('⚠️ Tidak ada akun expired yang ditemukan.', { show_alert: true });
      }
    });

  } catch (error) {
    logger.error('Error hapus_akun_expired:', error);
...
async function handleDepositState(ctx, userId, data) {
  ...
  const statusReseller = isReseller ? 'Reseller' : 'Bukan Reseller';
  const minDeposit = isReseller ? 20000 : 1000;
...
async function sendPaymentSuccessNotification(userId, deposit, currentBalance, bonusNominal = 0) {
  try {
    // Hitung admin fee
    const adminFee = deposit.amount - deposit.originalAmount;
    const totalDiterima = deposit.originalAmount + bonusNominal;

    await bot.telegram.sendMessage(userId,
      `✅ *Pembayaran Berhasil!*\n\n` +
      `💰 Jumlah Deposit: Rp ${deposit.originalAmount.toLocaleString('id-ID')}\n` +
      `🎁 Bonus Topup: Rp ${bonusNominal.toLocaleString('id-ID')}\n` +
      `💵 Total Saldo Masuk: Rp ${totalDiterima.toLocaleString('id-ID')}\n` +
...
async function processMatchingPayment(deposit, matchingTransaction, uniqueCode) {
  ...
  const isReseller = await isUserReseller(deposit.userId);
  // Persentase bonus: 15% untuk reseller, 5% untuk non-reseller
  const persentaseBonus = isReseller ? 0.15 : 0.05;
  // Hitung nominal bonus (dibulatkan ke bawah agar tidak ada desimal)
  const bonusNominal = Math.floor(deposit.originalAmount * persentaseBonus);
  // Total saldo yang akan dimasukkan ke database
  const totalSaldoDiterima = deposit.originalAmount + bonusNominal;
  // ===========================
    ...
    db.serialize(() => {
      ...
      db.get('SELECT id FROM transactions WHERE reference_id = ?', 
        ...
        (err, row) => {
          ...
          // Update user balance (MENGGUNAKAN SALDO + BONUS)
          db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', 
            [totalSaldoDiterima, deposit.userId], 
            function(err) {
              if (err) {
                db.run('ROLLBACK');
              ...
              db.run(
                ...
                [deposit.userId, totalSaldoDiterima, 'deposit', providerReference, Date.now()],
                (err) => {
                  if (err) {
                    db.run('ROLLBACK');
                  ...
                  db.get('SELECT saldo FROM users WHERE user_id = ?', [deposit.userId], async (err, user) => {
                    ...
                    const notificationSent = await sendPaymentSuccessNotification(
                      ...
                      deposit,
                      user.saldo,
                      bonusNominal // Kirim info bonus ke fungsi struk
                    );
...
✅ <b>Top Up Berhasil</b>
👤 User: ${userDisplay}
💎 Status: <b>${isReseller ? 'Reseller' : 'Member'}</b>
💰 Nominal: <b>Rp ${deposit.originalAmount.toLocaleString('id-ID')}</b>
🎁 Bonus: <b>Rp ${bonusNominal.toLocaleString('id-ID')}</b>
🏦 Saldo Sekarang: <b>Rp ${user.saldo.toLocaleString('id-ID')}</b>
