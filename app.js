...
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  const passwordDariTombol = ctx.match[1]; // Bot sekarang TAU password akunnya dari tombol ini
  const userIdUser = ctx.from.id;

  // Mencari domain yang tepat di database berdasarkan password dan user
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [passwordDariTombol, userIdUser], async (err, row) => {
    if (err || !row) {
      return ctx.answerCbQuery('❌ Akun tidak ditemukan di database!', { show_alert: true });
    }

    await ctx.answerCbQuery('🔨 Merakit Config .ZIV...');

    try {
      const crypto = require('crypto');
      
      // 1. RAKIT SEMUA DATA KE DALAM SATU OBJEK (JANGAN DIPISAH)
      const dataFullZiv = {
      ...
      // 2. UBAH JADI STRING JSON (null, 2 WAJIB UNTUK ZIVPN)
      const rawConfig = JSON.stringify(dataFullZiv, null, 2);

      const secretKey = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12); // WAJIB 12 BYTE UNTUK AES-GCM
      
      // LOGIKA PBKDF2 (Sesuai contoh Python Abang)
      const key = crypto.pbkdf2Sync(secretKey, salt, 1000, 16, 'sha256');
      ...
      const finalResult = `${salt.toString('base64')}.${nonce.toString('base64')}.${blob.toString('base64')}\n`;

      await ctx.replyWithDocument(
        { source: Buffer.from(finalResult, 'utf-8'), filename: `Aryus_${passwordDariTombol}.ziv` },
        { caption: `✅ *Config ZIVPN Berhasil Dibuat!*\nHost: \`${row.domain}\`\n\nIsi file dijamin lengkap 100%.`, parse_mode: 'Markdown' }
      );
    } catch (e) {
      console.error("ZIV Error:", e);
      ctx.reply("❌ Gagal merakit file.");
    }
  });
});
