...
//
  if (state.step?.startsWith('username_del_')) {
    ...
    // Mengambil tipe server dan ID dari state
    const { type, serverId } = state;
    delete userState[ctx.chat.id];

    // === PROSES AMBIL DATA UNTUK REFUND (BERLAKU UNTUK SEMUA USER) ===
    db.get(`
    ...
    `, [ctx.from.id, username, serverId], async (errRow, row) => {
      ...
      // Hitung sisa hari dan total refund jika akun ditemukan
      if (!errRow && row && row.expired_at) {
        const now = Date.now();
        if (row.expired_at > now) {
          // Konversi sisa ms ke hari (1 hari = 86400000 ms), dibulatkan ke bawah
      ...
      try {
        ...
        // Kirim pesan status penghapusan dari script bash/API
        await ctx.reply(msg, { parse_mode: 'Markdown' });
        logger.info(`✅ Akun ${type} berhasil dihapus oleh user ${ctx.from.id}`);

        // Jika penghapusan akun SUKSES (tidak ada logo ❌ di pesan responsnya)
        if (!msg.includes('❌')) {
          
          // 1. Hapus dari database vpn_accounts
          db.run('DELETE FROM vpn_accounts WHERE user_id = ? AND password = ? AND server_id = ?', [ctx.from.id, username, serverId]);
          
          // 2. Tambahkan saldo refund ke user jika sisa hari > 0
          if (refundAmount > 0) {
            db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [refundAmount, ctx.from.id], (updateErr) => {
              if (!updateErr) {
                ctx.reply(`💸 *Refund Saldo Berhasil!*\n\n` +
                          ...
                          { parse_mode: 'Markdown' });
                logger.info(`Refund saldo sebesar Rp${refundAmount} ke user ${ctx.from.id} karena menghapus akun ${username}`);
              } else {
                logger.error('❌ Gagal update saldo refund:', updateErr.message);
              }
            });
          }
        }
...
if (state.step?.startsWith('password_')) {
  ...
  await ctx.reply('⏳ *Masukkan masa aktif (hari):*', { parse_mode: 'Markdown' });
      ...
      db.get('SELECT quota, iplimit, harga FROM Server WHERE id = ?', [state.serverId], async (err, server) => {
       ...
       // ==========================================
       // FITUR CREATE FILE .ZIV
       // ==========================================
       
       // 1. Susun raw data config-nya (sesuaikan dengan format asli ZIVPN sebelum dienkripsi)
       // Biasanya berbentuk JSON atau format teks khusus.
       const rawConfig = JSON.stringify({
         Host: rowServer.domain,
         Password: state.password,
         ISP: "PT Biznet Gio Nusantara",
         // ... tambahkan parameter lain yang dibutuhkan aplikasi ...
       });

       // 2. Enkripsi rawConfig menjadi string acak seperti isi file ProjectAryus.ziv
       // CATATAN: Kamu harus punya/membuat fungsi enkripsinya sendiri agar bisa terbaca di aplikasi ZIVPN.
       let encryptedConfig = rawConfig; // <-- GANTI INI DENGAN FUNGSI ENKRIPSI ZIVPN KAMU
       // Contoh: let encryptedConfig = encryptToZIVPN(rawConfig);

       // 3. Ubah string menjadi Buffer (file) dan beri nama file
       const fileBuffer = Buffer.from(encryptedConfig, 'utf-8');
       const fileName = `ProjectAryus_${state.password}.ziv`;

       // 4. Kirim file ke Telegram user
       await ctx.replyWithDocument(
         ...
         { 
           caption: `📄 *Config ZIVPN Auto-Generated*\nSilakan import file ini ke aplikasi ZIVPN.`, 
           parse_mode: 'Markdown' 
         }
       );
       // ==========================================
