...
//
// === ACTION GENERATE FILE .ZIV SAKTI ===
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  const passZiv = ctx.match[1];
  const userZiv = ctx.from.id;

  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [passZiv, userZiv], async (err, rowZiv) => {
    if (err || !rowZiv) return ctx.answerCbQuery('❌ Akun tidak ditemukan.', { show_alert: true });
    ...
    try {
      ...
      // Menggunakan nama variabel yang unik agar tidak tertukar
      const isiSuratZiv = {
      ...
      // null, 2 BIAR RAPI KE BAWAH (SYARAT MUTLAK ZIVPN)
      const teksJSON = JSON.stringify(isiSuratZiv, null, 2);

      const secretPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12); // STANDAR ANDROID 12 BYTE
      const key = crypto.pbkdf2Sync(secretPass, salt, 1000, 16, 'sha256');
      
      const cipher = crypto.createCipheriv('aes-128-gcm', key, nonce);
      let ciphertext = cipher.update(teksJSON, 'utf8');
      ciphertext = Buffer.concat([ciphertext, cipher.final()]);
      const tag = cipher.getAuthTag();
      const blob = Buffer.concat([ciphertext, tag]);
      const finalString = `${salt.toString('base64')}.${nonce.toString('base64')}.${blob.toString('base64')}\n`;

      await ctx.replyWithDocument(
        { source: Buffer.from(finalString, 'utf-8'), filename: `Aryus_${passZiv}.ziv` },
        { caption: `📄 *Config ZIVPN Berhasil!*\nHost: \`${rowZiv.domain}\`\n\nImport ke aplikasi ZIVPN`, parse_mode: 'Markdown' }
      );
    } catch (e) {
      console.error("Error ZIV:", e);
      ctx.reply("❌ Gagal merakit file .ziv.");
    }
  });
});
