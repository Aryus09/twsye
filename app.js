...
let keyboard;
  keyboard = [
    ...
    [
      { text: '👤 Akun Saya', callback_data: 'akun_saya' } // Tombol Baru
    ],
    [
...
bot.action('akun_saya', async (ctx) => {
  try {
    ...
    // 1. Ambil data saldo dari database
    let saldo = 0;
    try {
      const row = await new Promise((resolve, reject) => {
        db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], (err, row) => {
    ...
    // 2. Cek status Reseller
    let isReseller = false;
    if (fs.existsSync(resselFilePath)) {
      const resellerList = fs.readFileSync(resselFilePath, 'utf8').split('\n').map(x => x.trim());
      isReseller = resellerList.includes(userId.toString());
    }
    const statusReseller = isReseller ? '✅ Reseller Aktif' : '❌ Bukan Reseller';

    // 3. Format pesan untuk Akun Saya
    const pesanAkun = `
╭─ <b>👤 INFORMASI AKUN SAYA</b>
...
<i>Jika kamu ingin menambah saldo, silakan klik tombol TopUp di bawah ini.</i>
──────────────────────────`;

    // 4. Tombol navigasi di dalam menu Akun Saya
    const keyboardAkun = [
      [{ text: '💰 TopUp Saldo', callback_data: 'topup_saldo' }],
      [{ text: '🔙 Kembali ke Menu Utama', callback_data: 'send_main_menu' }]
    ];

    // 5. Ubah pesan yang sedang dilihat user menjadi pesan Akun Saya
    await ctx.editMessageText(pesanAkun, {
      parse_mode: 'HTML',
      reply_markup: { inline_keyboard: keyboardAkun }
    });

  } catch (error) {
    logger.error('Error saat menampilkan menu Akun Saya:', error);
    try {
      await ctx.reply('❌ Terjadi kesalahan saat memuat data akun kamu. Coba lagi nanti.');
    } catch (e) {}
  }
});
