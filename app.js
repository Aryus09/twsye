...
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [password, userId], async (err, acc) => {
    if (err || !acc) return ctx.answerCbQuery('❌ Gagal membuat file. Akun tidak ditemukan.', { show_alert: true });
    await ctx.answerCbQuery('Sedang membuat config .ziv...');

    try {
      const crypto = require('crypto');
      
      // Pastikan semua data masuk ke sini
      const configData = {
         ...
         "data": {
             ...
             "file.askHwid": "0",
             "alpha": "0",
             "udpauth": password, // Mengambil password dari pemicu tombol
             "beta": "0",
             "psibuiltin": "0",
             "file.askHwidLocal": "0",
             "file.askLogin": "0",
             ...
             "tunnelType": "4",
             "udpwindow": "2",
             "udpserver": acc.domain, // Mengambil domain dari database
             "gamma": "0",
             "file.askUser": "0",
             "psiauth": "",
             "litehost": "",
      ...
      // Gunakan null, 2 agar Pretty Print (Rapi ke bawah)
      const rawConfig = JSON.stringify(configData, null, 2);

      const secretPass = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12); // Wajib 12 byte untuk Android
      ...
      await ctx.replyWithDocument(
        { source: Buffer.from(finalZivString, 'utf-8'), filename: `Aryus_${password}.ziv` },
        {
          caption: `📄 *Config ZIVPN Berhasil Dibuat*\nHost: \`${acc.domain}\`\n\nSilakan import langsung ke aplikasi ZIVPN.`,
          parse_mode: 'Markdown'
        }
      );
    } catch (e) {
      console.error("Error generating ZIV:", e);
      ctx.reply("❌ Gagal merakit file .ziv, ada kesalahan sistem.");
    }
