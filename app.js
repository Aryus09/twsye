...
//
// === ACTION GENERATE FILE .ZIV FINAL ANTI-KOSONG ===
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [password, userId], async (err, acc) => {
    if (err || !acc) return ctx.answerCbQuery('❌ Akun tidak ditemukan.', { show_alert: true });
    ...
    try {
      ...
      // Pakai nama variabel unik biar nggak ketimpa script lain
      const DataConfigZiv = {
      ...
      // Pastikan stringify mengambil variabel DataConfigZiv yang benar
      const rawConfigFinal = JSON.stringify(DataConfigZiv, null, 2);
      ...
      let ciphertext = cipher.update(rawConfigFinal, 'utf8');
      ...
      await ctx.replyWithDocument(
        ...
        { caption: `📄 *Config ZIVPN Berhasil Dibuat*\nHost: \`${acc.domain}\`\n\nSilakan import ke aplikasi ZIVPN.`, parse_mode: 'Markdown' }
      );
    } catch (e) {
