...
bot.action(/^get_ziv_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT domain FROM vpn_accounts WHERE password = ? AND user_id = ? ORDER BY id DESC LIMIT 1', [pZiv, uZiv], async (err, row) => {
    if (!row) return ctx.answerCbQuery('❌ Gagal!');
    try {
      const crypto = require('crypto');
      const konfigurasiAryus = {
        "Author": "https://t.me/aryusZIVPN",
        "VERSI": "FINAL_FIX_BISMILLAH",
        "ok": true,
        "ext": "ziv",
        "data": {
          "sshPortaLocal": "1080",
      ...
      const textJSON = JSON.stringify(konfigurasiAryus, null, 2);
      const sKey = "SecurePart1SecurePart2SecurePart3SecurePart4SecurePart5";
      const salt = crypto.randomBytes(16);
      const nonce = crypto.randomBytes(12);
      const key = crypto.pbkdf2Sync(sKey, salt, 1000, 16, 'sha256');
      const cipher = crypto.createCipheriv('aes-128-gcm', key, nonce);
      let encrypted = cipher.update(textJSON, 'utf8');
      encrypted = Buffer.concat([encrypted, cipher.final()]);
      const tag = cipher.getAuthTag();
      const blob = Buffer.concat([encrypted, tag]);
      const result = `${salt.toString('base64')}.${nonce.toString('base64')}.${blob.toString('base64')}\n`;
      await ctx.replyWithDocument({ source: Buffer.from(result, 'utf-8'), filename: `Aryus_FIX_${pZiv}.ziv` });
